...
def create_labeled_test_and_train_buckets(data_frame: pd.DataFrame, test_bucket_size: float = 0.5,
    ...
    if isinstance(test_bucket_size, float):
        train_bucket = data_frame.sample(frac=1 - test_bucket_size)
...
def _create_an_user_ngram(list_of_an_user_word_representation: pd.DataFrame, amount_of_ngrams: int, n: int) -> np.array:
    """ Function that create n-grams for an label (user). """
    try:
    ...
    # in case amount_of_ngrams is too high (amount_of_ngrams > len(permutations) assign each item.
        ...
        n_grams_idxs = list(
            (islice(permutations(range(len(list_of_an_user_word_representation)), n), amount_of_ngrams)))
...
def create_users_ngrams(list_of_word_representation: np.array, list_of_labels: np.array,
    ...
    # temporary dataframe to concatenate label with word representations (that should be scaled)
    tmp_df = pd.concat(
    ...
    for k in tmp_df['user_label'].unique():
        ...
        if list_of_word_representation is not None:
            list_of_word_representation = np.concatenate([list_of_word_representation, _create_an_user_ngram(
                tmp_df[tmp_df['user_label'] == k].iloc[:, :-1].reset_index(drop=True), amount_of_ngrams_per_user, n)])
        else:
            list_of_word_representation = _create_an_user_ngram(
                tmp_df[tmp_df['user_label'] == k].iloc[:, :-1].reset_index(drop=True), amount_of_ngrams_per_user, n)
        list_of_labels = list_of_labels + [k for _ in range(amount_of_ngrams_per_user)]
    logger.info(
